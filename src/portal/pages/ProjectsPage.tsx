    ...
    <div className="space-y-6">
      <PortalCard className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-3">
          <div className="font-heading text-2xl font-extrabold text-text">Projects</div>
          <span className="rounded-full border border-border bg-card hover:bg-slate-50 px-3 py-1 text-xs font-extrabold text-text/75">
            {total}
          </span>
        </div>
              ...
              {paged.map((p, idx) => {
                ...
                return (
                  <tr key={p.id} className="text-secondary">
                    <td className="px-5 py-4 text-muted">{(page - 1) * pageSize + idx + 1}</td>
                    <td className="px-5 py-4">
                      <div className="font-extrabold text-text">{p.name}</div>
                      <div className="mt-1 text-xs font-semibold text-muted">{p.city}</div>
                    </td>
      ...
      {rows.length > 0 && (
        <div className="flex flex-col items-center justify-between gap-3 sm:flex-row">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="rounded-xl border border-border bg-card hover:bg-slate-50 px-4 py-2 text-sm font-extrabold text-text">
              {page} / {pageCount}
            </div>
      ...
      >
        <div className="grid gap-4 md:grid-cols-2">
          <div className={touched && !valid.name ? 'rounded-2xl border border-red-500/40 p-3' : ''}>
            <label className="text-sm font-semibold text-muted">Project Name*</label>
          ...
          <div className={`md:col-span-2 ${touched && !valid.location ? 'rounded-2xl border border-red-500/40 p-3' : ''}`}>
            ...
            <input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="e.g., Mumbai, Maharashtra" className="mt-2 w-full rounded-xl border border-border bg-card hover:bg-slate-50 px-4 py-3 text-sm text-text outline-none ring-orange-500/35 focus:ring-2" />
          </div>
          <div className={touched && !valid.budget ? 'rounded-2xl border border-red-500/40 p-3' : ''}>
            <label className="text-sm font-semibold text-muted">Budget (₹)*</label>
            <input value={budget} onChange={(e) => setBudget(e.target.value)} type="number" className="mt-2 w-full rounded-xl border border-border bg-card hover:bg-slate-50 px-4 py-3 text-sm text-text outline-none ring-orange-500/35 focus:ring-2" />
          </div>
          <div className={touched && !valid.startDate ? 'rounded-2xl border border-red-500/40 p-3' : ''}>
            ...
            <input value={startDate} onChange={(e) => setStartDate(e.target.value)} type="date" className="mt-2 w-full rounded-xl border border-border bg-card hover:bg-slate-50 px-4 py-3 text-sm text-text outline-none ring-orange-500/35 focus:ring-2" />
          </div>
          <div className={touched && !valid.deadline ? 'rounded-2xl border border-red-500/40 p-3' : ''}>
            <label className="text-sm font-semibold text-muted">Expected End Date*</label>
            <input value={deadline} onChange={(e) => setDeadline(e.target.value)} type="date" className="mt-2 w-full rounded-xl border border-border bg-card hover:bg-slate-50 px-4 py-3 text-sm text-text outline-none ring-orange-500/35 focus:ring-2" />
          </div>
          <div className="md:col-span-2">
            <label className="text-sm font-semibold text-muted">Description</label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="mt-2 w-full rounded-xl border border-border bg-card hover:bg-slate-50 px-4 py-3 text-sm text-text outline-none ring-orange-500/35 focus:ring-2" />
          </div>
